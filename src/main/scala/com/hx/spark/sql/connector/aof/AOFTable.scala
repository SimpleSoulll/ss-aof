 ...
 * @author AC
 */
private[aof] class AOFTable(_schema: StructType, config: AOFReadConfig) extends Table with SupportsRead {

  override def name(): String = config.path

  override def capabilities(): util.Set[TableCapability] = util.Set.of[TableCapability](TableCapability.MICRO_BATCH_READ, TableCapability.BATCH_READ)

  override def schema(): StructType = _schema

  private val stream = new AOFMicroBatchStream(config)

  private val batch = new AOFBatchReader(config)
